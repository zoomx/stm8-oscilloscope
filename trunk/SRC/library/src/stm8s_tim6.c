...
ITStatus TIM6_GetITStatus(TIM6_IT_TypeDef TIM6_IT)
{
    ...
    u8 itStatus = 0x0, itEnable = 0x0;

    /* Check the parameters */
    assert_param(IS_TIM6_GET_IT_OK(TIM6_IT));

    itStatus = (u8)(TIM6->SR1 & (u8)TIM6_IT);

    itEnable = (u8)(TIM6->IER & (u8)TIM6_IT);

    if ((itStatus != (u8)RESET ) && (itEnable != (u8)RESET ))
    {
...
/**
  ...
  * @param[in] TIM6_EventSource : Specifies the event source.
...
{
    /* Check the parameters */
    assert_param(IS_TIM6_EVENT_SOURCE_OK((u8)TIM6_EventSource));

    /* Set the event sources */
    TIM6->EGR |= (u8)TIM6_EventSource;
}
...
/**
  ...
  * @param[in] TIM6_IT : Specifies the pending bit to clear.
 * This parameter can be one of the @ref TIM6_IT_TypeDef enumeration.
  * @retval None
  * @par Required preconditions:
  * None
...
void TIM6_ClearITPendingBit(TIM6_IT_TypeDef TIM6_IT)
{
    ...
    TIM6->SR1 &= (u8)(~(u8)TIM6_IT);
}

/**
  * @brief Selects the TIM6 Trigger Output Mode.
  * @param[in] TIM6_TRGOSource : Specifies the Trigger Output source.
...
void TIM6_SelectOutputTrigger(TIM6_TRGOSource_TypeDef TIM6_TRGOSource)
{
    ...
    /* Reset the MMS Bits */
    tmpcr2 &= (u8)(~TIM6_CR2_MMS);

    /* Select the TRGO source */
    tmpcr2 |=  (u8)TIM6_TRGOSource;
...
/**
  ...
  * @param[in] TIM6_InputTriggerSource : Specifies Input Trigger source.
...
void TIM6_SelectInputTrigger(TIM6_TS_TypeDef TIM6_InputTriggerSource)
{
    ...
    tmpsmcr &= (u8)(~TIM6_SMCR_TS);
    tmpsmcr |= (u8)TIM6_InputTriggerSource;

    TIM6->SMCR = (u8)tmpsmcr;
}
...
void TIM6_InternalClockConfig(void)
{
    ...
    TIM6->SMCR &=  (u8)(~TIM6_SMCR_SMS);
}

/**
  * @brief Selects the TIM6 Slave Mode.
  * @param[in] TIM6_SlaveMode : Specifies the TIM6 Slave Mode.
...
void TIM6_SelectSlaveMode(TIM6_SlaveMode_TypeDef TIM6_SlaveMode)
{
    ...
    /* Reset the SMS Bits */
    tmpsmcr &= (u8)(~TIM6_SMCR_SMS);

    /* Select the Slave Mode */
    tmpsmcr |= (u8)TIM6_SlaveMode;
